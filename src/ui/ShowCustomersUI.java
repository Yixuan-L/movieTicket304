...
public class ShowCustomersUI extends JFrame {
    ...
    public void showFrame() {
        ...
        for (int i = 0; i < models.length; i++) {
            tableContent[i][0] = models[i].getCustomerid().strip();
            tableContent[i][1] = models[i].getPhone().strip();
            tableContent[i][2] = models[i].getName().strip();
            tableContent[i][3] = models[i].getBirthday().strip();
            tableContent[i][4] = models[i].getAddress().strip();
            tableContent[i][5] = models[i].getEmail().strip();
        }
        String[] names = {
                "CustomerID",
                "Phone",
